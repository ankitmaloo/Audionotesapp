...
final class AudioCapService: ObservableObject {
    ...
    func startRecording(systemAudioURL: URL, microphoneURL: URL) throws {
        ...
        // Try to setup system audio recording if we have an active process
        if let activeProcess = processController.activeProcess {
            do {
                ...
                logger.info("Started system audio recording for process: \(activeProcess.name)")
            } catch {
                logger.error("System audio recording failed: \(error, privacy: .public)")
                // Continue with microphone only
            }
        } else {
            logger.info("No active process found - recording microphone only")
        }
        
        // Always setup microphone recording
        ...
        logger.info("Started recording (microphone always enabled)")
    }
